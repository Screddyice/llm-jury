...
#
# On this 36 GiB host the Google/Meta pair projects ~21.0 GiB at num_ctx 8192
# and OLLAMA_NUM_PARALLEL=2, under the 23.4 GiB residency budget. Adding
# phi4-mini:3.8b projects ~26.5 GiB once its separate prompt cache is counted;
# that former three-model default was always refused by the live preflight.
# Keep the Microsoft model available as an explicit --models choice on larger
# hosts. The verifier accepts only passing candidates, so a smaller local panel
# may escalate more often but does not relax correctness.
#
# The full benchmark-mirrored council remains CLOUD_PANEL on OpenRouter when
# explicitly selected. Local code tasks use the safe pair, then a chosen
# verifier-gated frontier such as the authenticated Codex CLI.
LOCAL_PANEL = ["gemma3:12b", "llama3.1:8b"]
