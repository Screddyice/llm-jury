...
def test_agent_frontmatter_description_is_quoted_for_strict_yaml():
    """Claude Code parses agent frontmatter leniently. Other hosts do not.

    Under a strict YAML parser an unquoted `description:` containing a
    colon-space is not a string — it reads as a mapping and the agent is
    silently dropped, with no error anywhere. A retired host burned a day on
    exactly that. Quoting costs nothing and keeps the shipped agent loadable
    wherever it lands.
    """
    ...
    if ": " in value:
        assert value.startswith('"') and value.endswith('"'), (
            "a description containing ': ' must be a quoted YAML scalar or a "
            "strict-YAML host will silently drop the agent")
